...
export function normalizeTileData(
  ...
  if (tile.geometryData) {
    contentUrl = `${url}/${tile.geometryData[0].href}/index.json`;
  }

  let textureUrl: string | undefined;
  if (tile.textureData) {
    textureUrl = `${url}/${tile.textureData[0].href}/index.json`;
  }
...
export async function normalizeTilesetData(
  ...
  if (tileset.nodePages) {
    ...
    const rootNodeUrl = getUrlWithToken(`${tileset.url}/nodes/root/index.json`, options.i3s?.token);
